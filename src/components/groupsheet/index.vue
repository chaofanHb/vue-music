...
<template>
  <div class="group-sheet">
    <div class="title"><span>{{sheetName}}</span><i class="icon-circle-right"></i></div>
    <div class="content" v-if="sheetList.length">
      <router-link class="sheet-list" v-for="(item, index) in sheetList" v-if="index &lt; 6" :to="{path: '/main/listdetail', query: {id: item.id}}" :key="item.id">
        <div class="sheet-image"><img :src="item.picUrl" />
          <div class="high-quality" v-if="item.highQuality"><i class="icon-menu"></i></div>
          <div class="tips"><i class="icon-menu"></i><span>{{item.playCount | parseNumber}}</span></div>
        </div>
