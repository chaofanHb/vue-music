// play页面呼起的列表内容
<template>
  <div class="music-sider-lists">
    <transition name="fade">
      <div class="mask" @click="close"></div>
    </transition>
    <transition name="sider-top">
      <div class="sider-content">
        <MusicList v-for="(item, index) in musicPlayLists" :key="item.id" :index="index" :name="item.name" :singer="item.ar" :id="item.id" :list="item" :playSheet="playSheet(item.id)" @play="playMusic"></MusicList>
      </div>
    </transition>
  </div>
</template>
