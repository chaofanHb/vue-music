<template>
  <div class="lrc-info" ref="lrc">
    <div class="lrc-c" v-if="hasLrc === 1">
      <div class="lrc-list" v-for="(item, index) in lrcs" :class="{active: activeIndex === index}">{{item.lrc}}</div>
    </div>
    <div class="lrc-tip" v-if="hasLrc === 2">纯音乐，请欣赏</div>
    <div class="lrc-tip" v-if="hasLrc === 0">歌词加载中, 请稍等...</div>
    <div class="lrc-tip" v-if="hasLrc === 3">暂无歌词</div>
  </div>
</template>
