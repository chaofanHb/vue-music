// 搜索的歌单显示效果
<template>
  <router-link class="s-album" v-if="list &amp;&amp; list.name" :to="{path: '/main/listdetail', query: {id: list.id, type: 'album'}}">
    <div class="img"><img v-imgsize="list.picUrl" /></div>
    <div class="right-info">
      <div class="name">{{list.name}}</div>
      <div class="disc">
        <div class="user">{{getArtists}}</div>
        <div class="push-t">{{getDate}}</div>
      </div>
    </div>
  </router-link>
</template>
