...
<template>
  <div class="search-panel">
    <div class="search-lists-a">
      <div class="a-name">热门搜索</div>
      <div class="search-lists" v-if="hots.length">
        <div class="list" v-for="item in hots" @click="searchItem(item.first)">
          <div class="name">{{item.first}}</div>
          <div class="tip" v-if="item.iconType === 1">热</div>
        </div>
      </div>
      <Loading v-else="v-else"></Loading>
    </div>
    <div class="search-lists-a">
      <div class="a-name config">
        <div class="name">搜索历史</div><i class="icon-menu easy-click" @click="deleteHistory"></i>
      </div>
      <div class="search-lists" v-if="history.length">
        <div class="list" v-for="list in history" @click="searchItem(list)">
          <div class="name">{{list}}</div>
        </div>
      </div>
      <div class="none-list" v-else="v-else">暂无搜索记录</div>
    </div>
  </div>
</template>
