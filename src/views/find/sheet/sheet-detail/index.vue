...
<template>
  <div class="sheet-detail" v-if="!isEmptyDetail">
    <CommonPage title="歌单" :bg="imageSrc">
      <div class="content" slot="content">
        <div class="scroll-main">
          <div class="content-main">
            <div class="blur" :style="{backgroundImage: 'url(' + imageSrc + ')'}"></div>
            <div class="detail-main" v-if="isAlbum">
              <div class="sheet-avatar" @click="showSheetAvatar"><img class="avatar" v-imgsize="detail.blurPicUrl" />
                <div class="info">i</div>
              </div>
              <div class="sheet-disc">
                <div class="name">{{detail.name}}</div>
                <div class="user"><span>{{detail.artists[0].name}}</span><i class="icon-menu"></i></div>
              </div>
            </div>
            <div class="detail-main" v-else="v-else">
              <div class="sheet-avatar" @click="showSheetAvatar"><img class="avatar" v-imgsize="detail.coverImgUrl" />
                <div class="tips">{{detail.playCount | parseNumber}}</div>
                <div class="high-quality" v-if="detail.highQuality"><i class="icon-menu"></i></div>
                <div class="info">i</div>
              </div>
              <div class="sheet-disc">
                <div class="name">{{detail.name}}</div>
                <div class="user"><img v-imgsize="{url: detail.creator.avatarUrl, w: 120}" /><span>{{detail.creator.nickname}}</span><i class="icon-menu"></i></div>
              </div>
            </div>
            <div class="detail-conf">
              <div class="conf-list" v-if="!isAlbum"><i class="icon-menu"></i><span class="disc">{{detail.commentCount}}</span></div>
              <div class="conf-list" v-else="v-else"><i class="icon-menu"></i><span class="disc">{{detail.info.commentCount}}</span></div>
              <div class="conf-list" v-if="!isAlbum"><i class="icon-menu"></i><span class="disc">{{detail.shareCount}}</span></div>
              <div class="conf-list" v-else="v-else"><i class="icon-menu"></i><span class="disc">{{detail.info.shareCount}}</span></div>
              <div class="conf-list"><i class="icon-menu"></i><span class="disc">下载</span></div>
              <div class="conf-list"><i class="icon-menu"></i><span class="disc">多选</span></div>
            </div>
          </div>
          <div class="content-lists-info">
            <div class="list-title">
              <div class="play"><i class="icon-menu"></i><span class="label">播放全部</span><span class="count">(共{{detail.trackCount}}首)</span></div>
              <div class="collect"><span v-if="!detail.subscribed" @click="subscribe">+ 收藏 ({{detail.subscribedCount | parseNumber}})</span><span v-else="v-else" @click="subscribe">已收藏</span></div>
            </div>
            <div class="list-content">
              <MusicList v-for="(item, index) in tracks" :key="item.id" :index="index" :name="item.name" :singer="item.ar" :id="item.id" :list="item" :playSheet="playSheet(item.id)" @play="playMusic"></MusicList>
            </div>
          </div>
        </div>
      </div>
    ...
    <transition name="fade">
      <SheetAvatar v-if="showAvatar" @hideSheetAvatar="hideSheetAvatar" :avatarData="detail"></SheetAvatar>
    </transition>
  </div>
</template>
